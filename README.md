...
### `ButtonsSocialNetworks.js`

```js
import React, { Component } from 'react';

class ButtonsSocialNetworks extends Component {
  render() {
    // ...
  }
}

export default ButtonsSocialNetworks; // Don’t forget to use export default!
```
...
```js
import React, { Component } from 'react';
import ButtonsSocialNetworks from './ButtonsSocialNetworks'; // Import a component from another file

class DangerButton extends Component {
  render() {
    return <ButtonsSocialNetworks color="red" />;
  }
}
...
We suggest that you stick to using default imports and exports when a module only exports a single thing (for example, a component). That’s what you get when you use `export default ButtonsSocialNetworks` and `import ButtonsSocialNetworks from './ButtonsSocialNetworks'`.
...
```js
import React, { Component } from 'react';
import './ButtonsSocialNetworks.css'; // Tell Webpack that ButtonsSocialNetworks.js uses these styles

class ButtonsSocialNetworks extends Component {
  render() {
    // You can use them as regular CSS styles
    return <div className="ButtonsSocialNetworks" />;
  }
}
```
...
Generally, we recommend that you don’t reuse the same CSS classes across different components. For example, instead of using a `.ButtonsSocialNetworks` CSS class in `<AcceptButton>` and `<RejectButton>` components, we recommend creating a `<ButtonsSocialNetworks>` component with its own `.ButtonsSocialNetworks` styles, that both `<AcceptButton>` and `<RejectButton>` can render (but [not inherit](https://facebook.github.io/react/docs/composition-vs-inheritance.html)).
...
```js
import { Navbar, Jumbotron, ButtonsSocialNetworks } from 'react-bootstrap';
```
...
Unlike the previous smoke test using `ReactDOM.render()`, this test only renders `<App>` and doesn’t go deeper. For example, even if `<App>` itself renders a `<ButtonsSocialNetworks>` that throws, this test will pass. Shallow rendering is great for isolated unit tests, but you may still want to create some full rendering tests to ensure the components integrate correctly. Enzyme supports [full rendering with `mount()`](http://airbnb.io/enzyme/docs/api/mount.html), and you can also use it for testing state changes and component lifecycle.
